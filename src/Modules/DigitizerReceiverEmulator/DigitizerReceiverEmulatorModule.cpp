...
#include "DigitizerReceiverEmulatorModule.hpp"

DigitizerReceiverEmulatorModule::DigitizerReceiverEmulatorModule() { INFO(""); 
  INFO("DigitizerReceiverModule Constructor");
  //GETS THE SETTINGS FROM CINFIG GILE I THINK
  //I STILL WANT TO LOAD IN A CONFIG FILE SO KEEP THE DECLARATION
...
// THERE IS NOTHING TO CONFIGURE BUT DAQLING EXPECTS THIS
void DigitizerReceiverEmulatorModule::configure() {
...
void DigitizerReceiverEmulatorModule::start(unsigned int run_num) {
  INFO("Starting ...");
  ...
  // DONT NEED THIS, NO HARDWARE TO START ACQUISIITON WITH
...
void DigitizerReceiverEmulatorModule::stop() {
  FaserProcess::stop();
  INFO("Stopping ...");
  //DONT NEED NO HARDWARE
  //m_digitizer->StopAcquisition();
}

void DigitizerReceiverEmulatorModule::sendECR() {
...
void DigitizerReceiverEmulatorModule::runner() {
  INFO("Running...");  
  while (m_run) {    
    //NOT SENDING SOFTWARE TRIGGERS DONT NEED
...
void DigitizerReceiverEmulatorModule::sendEvent() {
  INFO("Sending Event...");
  ...
  //COMMENT OUT FOR NOW, MAY NEED FOR CONSTRUCTING FRAGEMENT LATER
  ...
  DEBUG("local_event_id   : "+to_string(local_event_id));
  DEBUG("local_bc_id : "+to_string(local_bc_id));
