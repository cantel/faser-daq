...
void DisruptorFileDataLogger::runner() 
{
    ...
    while (m_run)
    {
        ...
        l_eh = static_cast<const EventHeader *>(pl.startingAddress()); 
        m_eventTag = l_eh->event_tag; //Extracting the current event tag (the event tag of the current payload)
        int l_eventSize = l_eh->payload_size+l_eh->header_size;
        ...
        while (!m_stopWriters)
        {
            if (m_payloads.sizeGuess() > 0)
            {
                ...
                if (sizeSum > m_writeBytes) //Writing is ready 
                {
                    ...
                    m_fileBuffers[m_eventTag][l_bufferCounter] = daqutils::Binary(0); //reset buffer to avoid multiple copies.
                }
            }
...
{ 
    ...
    while(m_run)
    {
        if(m_bufferReady != -1)
        {
            ...
            if(m_fstreamFull != 1)
            {
                m_fileStreams[ftid][l_eventTag].first.write(static_cast<char *>(m_fileBuffers[l_eventTag][l_bufferReady].startingAddress()),
            ...
            {
                m_fileStreams[ftid][l_eventTag].second.write(static_cast<char *>(m_fileBuffers[l_eventTag][l_bufferReady].startingAddress()),
                                    m_fileBuffers[l_eventTag][l_bufferReady].size());  // write
...
/**
void DisruptorFileDataLogger::handleBadEvent(const EventHeader* badEvent)
{
